package com.yann.hibernate.nTo1;
...
public class HibernateTest {
	...
	public void testManyToOneSave() {
		...
		//推荐:先插1的一端后插多的一端	
		//如果反过来也可以, 但要多出update语句(ID还未确定,只能等Customer插入后再更新Order)
		session.save(c);
		session.save(o1);
		session.save(o2);
	}
	...
	public void testManyToOneGet() {
		//1.查多的一端的对象, 默认知查多的一端的对象. 不查询关联对象
		Order order = session.get(Order.class, 1);
		System.out.println(order.getOrderName());
		
		//2.只有再使用到关联对象时, 才发送相应SQL语句
		Customer customer = order.getCustomer();
		System.out.println(customer.getCustomerName());
		
		//3.在查询Customer对象时, 由多的一端导航到1的一端时, 如果session关闭, 则会发生LazyInitializationException 异常
		//4. 获取order对象时(get), Customer对象为代理对象(load)
	}
